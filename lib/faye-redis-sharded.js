...
Engine.prototype = {
  ...
  publish:function (message, channels, waitGroup) {
    var self = this;
    ...
    function publish(message, channels, shardManager) {
      ...
      Object.keys(shardMap).forEach(function (shardName) {
        ...
        self._publish(message, channels, shard, shardManager, waitGroup);
      });
    }
  ...
  _publish:function (message, channels, channelShard, shardManager, waitGroup) {
    var self = this,
    ...
    var notify = function (error, clients) {
      ...
      clients.forEach(function (clientId) {
        ...
        redis.rpush(self._ns + '/clients/' + clientId + '/messages', jsonMessage, function (err, result) {
          if (error) return done(error);
          redis.publish(self._ns + '/' + clientId + '/notify', clientId, function (err, result) {
            if (error) return done(error);
      ...
      // Everything queued now, we can mark our add as done
      done();
    };
...
Engine.WaitGroup = function(callback, context) {
  this.waiting = 0;
...
Engine.WaitGroup.prototype = {
  add:function () {
    ...
    if (this.waiting == 0 && this.callback) {
      this.callback.call(this.context, this.errors);
    }
  }
};
