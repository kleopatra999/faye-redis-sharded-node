...
Engine.prototype = {
  ...
  destroyClient:function (clientId, callback, context) {
    ...
    // If we are destroying client it's because we lost connection, (no connects in timeout) and are removing their state
    // unfortunately Faye has already cleaned up the connection resources but has not called Engine.Proxy.closeConnection
    // since it is only one logical transport that failed not the whole thing.
    // Corollary is that watching for conneciton:close won't work for dropped/timedout connections and leads to bad stats
    // plus there is a memory leak where Engine.Proxy.connections will never remove the state for the dropped connection
    // Server is actually the Engine.Proxy instance so we can make it clean up manually
  ...
  _publish:function (message, channels, channelShard, shardManager) {
    ...
    var notify = function (error, clients) {
      ...
      clients.forEach(function (clientId) {
        ...
        redis.rpush(self._ns + '/clients/' + clientId + '/messages', jsonMessage, function (error, result) {
          if (error) return;
          redis.publish(self._ns + '/' + clientId + '/notify', clientId, function (error, result) {
            if (error) return;
  ...
  emptyQueue:function (clientId, callback, context) {
    ...
    multi.exec(function(err, replies){
      if (callback) {
        callback.call(context, err, replies);
      }
      if (err) {
        err.forEach(function(err){
          self.onRedisError(err);
        });
      }
    });
  },
